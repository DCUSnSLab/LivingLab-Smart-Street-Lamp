...
class Annotator:
    ...
    def draw_centroid_log(self, box, pts, i, identities=None, color=(128, 128, 128)):
        id = int(identities[i]) if identities is not None else 0
        center = ((round((box[0] + box[2]) / 2), round((box[1] + box[3]) / 2)))
        pts[id].append(center)
        cv2.circle(self.im, (round((box[0] + box[2]) / 2), round((box[1] + box[3]) / 2)), 1, color, 5)

        for j in range(1, len(pts[id])):
            if pts[id][j - 1] is None or pts[id][j] is None:
                continue
            thickness = int(np.sqrt(64 / float(j + 1)) * 2)
            cv2.line(self.im, (pts[id][j - 1]), (pts[id][j]), color, thickness)
    ...
    def result(self, str_time, end_time):
        # Print FPS
        fps_label = "FPS: %.2f" % (1 / (end_time - str_time))
        cv2.putText(self.im, fps_label, (0, 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1)
